...
@section('content')
    <div class="container">
        <h1 class="text-primary pb-3">Editar un administrador</h1>
        <form action="{{ route('admin.administrator.update', $administrator) }}" method="POST" class="form">
            @method('PUT')
            @csrf
            ...
            <div class="form-group col-md-6 p-0">
                ...
                <input id="identification" name="identification" type="text" required placeholder="Ingresa la cédula"
                       value="{{ old('identification', $administrator->identification) }}"
            ...
            <div class="form-group col-md-6 p-0">
                ...
                <input id="address" name="address" type="text" placeholder="Ingresa la dirección"
                       value="{{ old('address', $administrator->address) }}"
            ...
            <div class="form-group col-md-6 p-0">
                ...
                <input id="birthday" name="birthday" type="text" placeholder="Ingresa la fecha de nacimiento"
                       value="{{ old('birthday', $administrator->birthday) }}"
            ...
            <div class="form-group col-md-6 p-0">
                ...
                <select id="gender" name="gender" class="form-control @error('gender') is-invalid @enderror"
                    ...
                    <option value="M" {{ old('gender', $administrator->gender) === 'M' ? 'selected' : '' }}>Masculino
                    </option>
                    <option value="F" {{ old('gender', $administrator->gender) === 'F' ? 'selected' : '' }}>Femenino
                    </option>
                </select>
                @error('gender')
            ...
            <div class="form-group col-md-6 p-0">
                ...
                <select id="city_id" name="city_id" class="form-control @error('city_id') is-invalid @enderror"
                    ...
                    @foreach($cities as $city)
                        <option
                            value="{{ $city->id }}" {{ old('city_id', $administrator->city_id) === $city->id ? 'selected' : '' }}>{{ $city->province->name }}
                            , {{ $city->name }}</option>
                    @endforeach
                </select>
                @error('city_id')
            ...
            <div class="pt-3">
                <a href="{{ route('admin.administrator.index') }}" class="btn btn-secondary">
                    <span>Cancelar</span>
                </a>
