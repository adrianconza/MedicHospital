...
@section('content')
    <div class="container">
        <div class="d-flex flex-row justify-content-between align-items-center pb-4">
            ...
            <div>
                <a href="{{ route('admin.client.create') }}" class="btn btn-primary px-4">
                    <span>Crear</span>
                </a>
            </div>
        </div>

        <form method="GET" action="{{ route('admin.client.index') }}" class="p-0 pb-3 col-md-6">
                        ...
                        <tr id="user-{{ $user->id }}">
                            ...
                            <td id="destroy" colspan="6" class="d-none">
                                <div class="d-flex flex-row justify-content-between align-items-center">
                                    <span>Estás seguro de <strong>Desactivar</strong> el cliente: <strong>{{ $user->name }} {{ $user->last_name }}</strong>?</span>
                                    <div class="dialog-destroy-btn">
                                        <form
                                            action="{{ route('admin.client.destroy', $user) }}"
                            ...
                            <td id="restore" colspan="6" class="d-none">
                                <div class="d-flex flex-row justify-content-between align-items-center">
                                    <span>Estás seguro de <strong>Activar</strong> el cliente: <strong>{{ $user->name }} {{ $user->last_name }}</strong>?</span>
                                    <div>
                                        <form
                                            action="{{ route('admin.client.restore', $user->id) }}"
                                            method="POST"
                                            class="d-inline">
                                            @method('PUT')
                                            @csrf
