...
@section('content')
    <div class="container">
        <h1 class="text-primary pb-3">Editar un cliente</h1>
        <form action="{{ route('admin.client.update', $client) }}" method="POST" class="form">
            @method('PUT')
            @csrf
            ...
            <div class="form-group col-md-6 p-0">
                ...
                <select id="city_id" name="city_id" class="form-control @error('city_id') is-invalid @enderror"
                    ...
                    @foreach($cities as $city)
                        <option
                            value="{{ $city->id }}" {{ +old('city_id', $client->city_id) === +$city->id ? 'selected' : '' }}>{{ $city->province->name }}
                            , {{ $city->name }}</option>
                    @endforeach
                </select>
                @error('city_id')
