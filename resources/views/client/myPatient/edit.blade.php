...
@section('content')
    <div class="container">
        <h1 class="text-primary pb-3">Editar mi paciente</h1>
        <form action="{{ route('client.myPatient.update', $patient) }}" method="POST" class="form">
            @method('PUT')
            @csrf
            ...
            <div class="form-group col-md-6 p-0">
                ...
                <select id="gender" name="gender" class="form-control @error('gender') is-invalid @enderror"
                    ...
                    <option value="M" {{ old('gender', $patient->gender) === 'M' ? 'selected' : '' }}>Masculino
                    </option>
                    <option value="F" {{ old('gender', $patient->gender) === 'F' ? 'selected' : '' }}>Femenino
                    </option>
                </select>
                @error('gender')
